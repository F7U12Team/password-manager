...
public class PMModule extends ReactContextBaseJavaModule {
    ...
    public PMModule(ReactApplicationContext reactContext) {
        ...
        Cerrojoandroid.registerJavaCallback(gocb);
    }
    ...
    public void getLogin(String id, Boolean edit, Promise promise) {
        try {
            Cerrojoandroid.getLogin(id, edit);
            WritableMap map = Arguments.createMap();
            promise.resolve(map);
        } catch (Exception e) {
            //Log.e(TAG, "Solve error: " + e.toString());
    ...
    public void saveLogin(String title, String username, String note, String password, String secretNote, String id, Promise promise) {
        try {
            Cerrojoandroid.saveLogin(title, username, note, password, secretNote, id);
            WritableMap map = Arguments.createMap();
            promise.resolve(map);
        } catch (Exception e) {
            //Log.e(TAG, "Solve error: " + e.toString());
    ...
    public static void sendToast(String text) {
        Toast.makeText(PMModule.rc, text, Toast.LENGTH_LONG).show();
    }

    @ReactMethod
    public void importLastPass(String file, Promise promise) {
        try {
            Cerrojoandroid.importFile(file, "lastpass");
            WritableMap map = Arguments.createMap();
            promise.resolve(map);
        } catch (Exception e) {
            //Log.e(TAG, "Solve error: " + e.toString());
